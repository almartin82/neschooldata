"""
Core functions wrapping neschooldata R package via rpy2.
"""
...
def _get_pkg():
    ...
    global _pkg
    if _pkg is None:
        _pkg = importr("neschooldata")
    return _pkg


def fetch_enr(end_year: int) -> pd.DataFrame:
    """
    Fetch Nebraska school enrollment data for a single year.
...
def fetch_enr_multi(end_years: list[int]) -> pd.DataFrame:
    """
    Fetch Nebraska school enrollment data for multiple years.
    ...
    --------
    >>> import pyneschooldata as ne
    >>> df = ne.fetch_enr_multi([2020, 2021, 2022])
    """
    pkg = _get_pkg()
