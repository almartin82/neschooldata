...
def get_test_years():
    ...
    if _available_years is None:
        ...
        _available_years = ne.get_available_years()
...
class TestImport:
    ...
    def test_import_functions(self):
        ...
        import pyneschooldata as ne
        assert hasattr(ne, 'fetch_enr')
        assert hasattr(ne, 'fetch_enr_multi')
        assert hasattr(ne, 'tidy_enr')
        assert hasattr(ne, 'get_available_years')

    def test_version_exists(self):
        """Package has a version string."""
        import pyneschooldata as ne
        assert hasattr(ne, '__version__')
        assert isinstance(ne.__version__, str)
...
class TestFetchEnr:
    ...
    def test_returns_dataframe(self):
        ...
        df = ne.fetch_enr(years['max_year'])
    ...
    def test_total_enrollment_reasonable(self):
        """Total enrollment is in a reasonable range for Nebraska."""
        import pyneschooldata as ne
        years = get_test_years()
        df = ne.fetch_enr(years['max_year'])
        # Total enrollment in Nebraska varies by grade level aggregation
        # Just check that total is positive and reasonable
        total = df['n_students'].sum()
        # Nebraska should have substantial enrollment
...
class TestFetchEnrMulti:
    ...
    def test_returns_dataframe(self):
        ...
        df = ne.fetch_enr_multi([years['max_year']])
        assert isinstance(df, pd.DataFrame)
    ...
    def test_contains_requested_year(self):
        ...
        df = ne.fetch_enr_multi([test_year])
    ...
    def test_multi_matches_single(self):
        ...
        import pyneschooldata as ne
        years = get_test_years()
        df_single = ne.fetch_enr(years['max_year'])
        df_multi = ne.fetch_enr_multi([years['max_year']])
