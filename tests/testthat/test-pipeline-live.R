# ==============================================================================
# LIVE Pipeline Tests for neschooldata
# ==============================================================================
#
...
test_that("Nebraska DOE website is accessible", {

  skip_if_offline()
  skip("TODO: Add Nebraska DOE URL and verify HTTP 200")
})
...
test_that("Can download Nebraska enrollment data file", {
  skip_if_offline()
...
test_that("Can parse Nebraska enrollment file", {
  skip_if_offline()
...
test_that("neschooldata data file has expected columns", {
...
test_that("get_raw_enr returns data for valid year", {
  ...
  # Get available years
  years_info <- neschooldata::get_available_years()
  ...
  tryCatch({
    raw <- neschooldata:::get_raw_enr(test_year)
...
test_that("get_available_years returns valid year range", {
  result <- neschooldata::get_available_years()
...
test_that("fetch_enr returns data with no Inf or NaN", {
  ...
  tryCatch({
    ...
    data <- neschooldata::fetch_enr(test_year, tidy = TRUE)
...
test_that("Enrollment counts are non-negative", {
  ...
  tryCatch({
    ...
    data <- neschooldata::fetch_enr(test_year, tidy = FALSE)
...
test_that("tidy=TRUE and tidy=FALSE return consistent totals", {
  ...
  tryCatch({
    ...
    wide <- neschooldata::fetch_enr(test_year, tidy = FALSE)
    tidy <- neschooldata::fetch_enr(test_year, tidy = TRUE)
...
test_that("Cache functions exist and work", {
  ...
  tryCatch({
    path <- neschooldata:::get_cache_path(2024, "enrollment")
